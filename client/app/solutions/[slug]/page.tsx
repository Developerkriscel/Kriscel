...
export default async function SolutionPage({ params }: { params: { slug: string } }) {
  const { slug } = params;
  const data = SOLUTIONS_DATA[slug as keyof typeof SOLUTIONS_DATA];
    ...
    <div className="min-h-screen pt-40 pb-20 px-6 bg-background">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <Link href="/contact" className="inline-block px-8 py-4 bg-accent text-white font-bold rounded-xl hover:bg-accent-hover transition-all shadow-lg shadow-accent/20">
            {data.cta}
          </Link>
        </div>
